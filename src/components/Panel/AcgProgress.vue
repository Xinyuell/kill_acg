...
const tips = computed(()=>{
  ...
  const time = (GlobalConfig.AcgProgressData.AcgProgressMax - store.state.gameData.acgProgressValue) / GlobalConfig.AcgProgressData.AcgProgressSpeed;
  
  let str = "ACG文化对世界造成的影响力" + intToString(store.state.gameData.acgProgressValue) + "；进度：" +  precent + "%\n";
  str += "ACG文化影响力增长速度" + intToString(GlobalConfig.AcgProgressData.AcgProgressSpeed) + "\n还需" + time.toFixed(1) + "天（现实时间1秒等于游戏时间一天）,世界将被毁灭!\n"
